...
class AdministrationNavigation {
  constructor() {
    ...
    this.orgnav = '.table tbody tr td:nth-child(3)';
  ...
  selectAdministration() {
    cy.get(this.orgnav).each(($element) => {
      const text = $element.text().trim();
      if (text === this.orgName) {
        cy.wrap($element).parent('tr').click();
      }
    });
  }
  ...
  backToAllCustomerError() {
    cy.contains('Back to All Organizations').click();
    cy.get(this.alert).should(
      'not.have.text',
      'Unknown Error - Please check your internet connection. If the problem persists, please contact support.'
    );
