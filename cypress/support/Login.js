...
class Login {
  constructor() {
    ...
    this.navMenu = '#dropdown-basic';
    this.signout = 'Logout';
    this.customer = 'Customers';
    this.loginPageIcon = '.form-section-heading > img';
    this.loginPageIconName = 'jobs page icon';
    this.jobsPageIcon = "img[alt='One Click Contractor']";
    this.jobsPageIconName = 'login page icon';
    this.favIcon = "link[rel='icon']";
  }

  setLoginEmailAndPasswordUsingApi(email = Cypress.env('adminUsername'), password = Cypress.env('adminPassword')) {
    cy.request({
      url: Cypress.env('url'),
      method: 'POST',
      form: true,
  ...
  iconsVerification(locartor, iconName) {
    cy.get(locartor).compareSnapshot(iconName);
  }

  favIconVerification() {
    cy.get(this.favIcon).should('have.attr', 'href').should('eq', '/assets/images/favicon.18877e0c.ico');
  }

  jobsExistsAssertion() {
    cy.get('h2:contains("No Customers Exist. Let\'s add one.")', { timeout: 3000 }).should('not.exist');
  }
}
