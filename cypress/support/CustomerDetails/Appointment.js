...
class Appointment {
  constructor() {
    ...
    this.email = '#email';
    this.send = '.modal-footer .btn.btn-primary';
    this.validationtext = '.block.invalid-feedback';
  }
  ...
  blankEmailFieldValidation() {
    cy.contains(this.sendinvite).click();
    cy.get(this.email).clear();
    cy.get(this.send).click();
    cy.get(this.validationtext).should('have.text', 'Email is required');
  }
