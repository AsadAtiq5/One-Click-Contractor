...
class Document {
  constructor() {
    ...
    this.proposalcheckbox = '.table-row > :nth-child(4) > .form-check > [data-testid="displayInProposal"]';
    this.agreementcheckbox = '.table-row > :nth-child(5) > .form-check > [data-testid="displayInAgreement"]';
    this.documentSavedSuccessMessage = '.alert-message';
    this.toDocument = '.btn-light';
    this.documentName = '.break-word.col';
  }
  ...
  uploadFileWithoutSignatures() {
    cy.get(this.upload).selectFile('cypress/fixtures/nosigners.pdf', {
      action: 'drag-drop'
    });
    cy.get(this.spinner, { timeout: 20000 }).should('not.exist');
    cy.contains('nosigners');
  }
  ...
  resource() {
    cy.get(this.resources).click({ force: true });
  ...
  navigateToFolder() {
    cy.get(this.childInsideChild, { timeout: 5000 }).first().click();
  ...
  clickMagicWand() {
    cy.get(this.magicWand).first().click();
    cy.wait(30000);
  ...
  verifyDocumentSaveSuccessMessage() {
    this.clickMagicWand();
    cy.get(this.documentSavedSuccessMessage, { timeout: 30000 })
      .should('be.visible')
      .and('have.text', 'Saved document successfully');
  }
  ...
  missingCheckBoxesProposalandAngreement() {
    cy.get(this.proposalcheckbox, { timeout: 10000 }).should('be.visible');

    cy.get(this.agreementcheckbox, { timeout: 10000 }).should('be.visible');
  }

  proposalCheck() {
    cy.get(this.proposalcheckbox).first().click();
  }

  agreementCheck() {
    cy.get(this.agreementcheckbox).first().click();
  }

  navigateFromResourcesToDocuments() {
    cy.get(this.toDocument).click();
  }

  verifyDocumentsNomenclature() {
    cy.get(this.documentName)
      .invoke('text')
      .then((text) => {
        const forbiddenCharacters = ['(', ')', ':'];
        const containsForbiddenCharacters = forbiddenCharacters.some((char) => text.includes(char));
        expect(containsForbiddenCharacters).to.be.false;
      });
  }
}
