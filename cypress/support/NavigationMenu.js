...
class NavigationMenu {
  constructor() {
    ...
    this.selectOrgProfileDropdown =
      'div.m-0.p-0.dropdown-item a.dropdown-item.text-decoration-none.leads-anchor-tag[href^="/orgs/accesses"]';
    ...
    this.totalCustomerCard = '.mb-2.statistics.row';
    this.graphLocator = '.col-6 canvas';
    this.graph = 'Graph';
    this.emptyGraph = 'Empty Graph';
  }
  ...
  totalCustomerStatistics() {
    cy.get(this.totalCustomerCard).should('not.exist');
  }

  graphVerification(graphName) {
    cy.wait(5000);
    cy.get(this.graphLocator).compareSnapshot(graphName);
  }
}
