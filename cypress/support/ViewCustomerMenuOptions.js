...
class ViewCustomerMenuOptions {
  constructor() {
    ...
    //signature tab
    this.signaturetab = '#signatures > .k-link';
    //Inspection tab
    this.inspectiontab = '#inspection > .k-link > .menu-text > .customer_nav_text';
  }
  ...
  viewInspection() {
    cy.get(this.inspectiontab).click();
    cy.get(this.spinner, { timeout: 50000 }).should('not.exist');
    cy.url().should('include', '/inspection');
  }
}
